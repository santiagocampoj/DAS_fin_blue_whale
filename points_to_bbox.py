import os
import numpy as np
import pandas as pd
from config import BBOX_SUFIX



def points_to_bbox(csv_path, output_path=None, output_name=None, group_col='call_id',
                   time_s=None, dist_m=None, start_datetime_utc=None, logger=None):
    """
    Convert point annotations to bounding boxes in events CSV format.

    Parameters
    ----------
    csv_path : str
        Path to the original annotation CSV file.
    output_path : str, optional
        Directory where the output CSV will be saved.
        Defaults to the same directory as the input CSV.
    output_name : str, optional
        Output filename. Defaults to input filename + '_bbox.csv'.
    group_col : str
        Column name to group by (default: 'call_id').

    Returns
    -------
    pd.DataFrame
        One row per group with columns:
        [ID, t0, t1, d0, d1, ti0, ti1, di0, di1, nx, nt, downsample, start_datetime_utc, comment]
    """
    df = pd.read_csv(csv_path)
    if logger:
        logger.info(f"Loaded {len(df)} rows from {csv_path}")

    # Derive axes and start time from the CSV itself
    time_s = np.sort(df['time'].unique())
    dist_m = np.sort(df['dist'].unique())
    start_datetime_utc = (pd.to_datetime(df['utc_time']) - pd.to_timedelta(df['time'], unit='s')).iloc[0]

    records = []
    for _, grp in df.groupby(group_col):
        t0 = grp['time'].min()
        t1 = grp['time'].max()
        d0 = grp['dist'].min()
        d1 = grp['dist'].max()

        ti0 = int(np.searchsorted(time_s, t0))
        ti1 = int(np.searchsorted(time_s, t1))
        nt  = len(time_s)

        di0 = int(np.searchsorted(dist_m, d0))
        di1 = int(np.searchsorted(dist_m, d1))
        nx  = len(dist_m)

        bbox_timestamp = (pd.Timestamp(start_datetime_utc) + pd.Timedelta(seconds=t0)).isoformat(timespec='microseconds')

        records.append({
            'ID':                 grp['call_type'].iloc[0],
            't0':                 t0,
            't1':                 t1,
            'd0':                 d0,
            'd1':                 d1,
            'ti0':                ti0,
            'ti1':                ti1,
            'di0':                di0,
            'di1':                di1,
            'nx':                 nx,
            'nt':                 nt,
            'downsample':         None,
            'start_datetime_utc': bbox_timestamp,
            'comment':            None,
        })

    df_bbox = pd.DataFrame(records)

    base_name = output_name if output_name else os.path.splitext(os.path.basename(csv_path))[0] + BBOX_SUFIX
    out_dir = output_path if output_path else os.path.dirname(csv_path) or '.'
    out_file = os.path.join(out_dir, base_name)

    # df_bbox.to_csv(out_file, index=False)
    if logger:
        logger.info(f"Saved bounding box CSV to: {out_file}")

    return df_bbox